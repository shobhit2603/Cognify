    ...
    <section className="relative w-full py-20 sm:py-28 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {workflowItems.map((item) => (
          <div
            key={item.id}
            className={`group rounded-3xl p-6 flex flex-col justify-between min-h-90 border shadow-xl transition-all duration-300 hover:-translate-y-2 hover:shadow-2xl ${item.color}`}
          >
            <div>
              <div className="flex items-center justify-between mb-4">
                ...
                <span className="text-[9px] font-mono font-extrabold uppercase px-2 py-0.5 rounded-full bg-black/15 tracking-wider">
                  {item.badge}
                </span>
              </div>
            ...
            <div className="pt-6 mt-6 border-t border-current/10 flex items-center justify-between">
              ...
              <Link
                href="/auth"
                className="w-8 h-8 rounded-full bg-white text-black flex items-center justify-center hover:scale-110 transition-transform shadow"
              >
