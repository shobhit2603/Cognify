...
const slideVariants = {
  enterFromRight: { opacity: 0, x: 32, scale: 0.98 },
  enterFromLeft: { opacity: 0, x: -32, scale: 0.98 },
  center: { opacity: 1, x: 0, scale: 1 },
  exitToLeft: { opacity: 0, x: -32, scale: 0.98 },
  exitToRight: { opacity: 0, x: 32, scale: 0.98 },
};
  ...
  return (
    ...
    >
      ...
      <button
        type="button"
        onClick={onBack}
        className="flex items-center gap-1.5 text-gray-400 hover:text-white text-xs font-sans transition-colors mb-5 cursor-pointer group"
      >
        ...
        >
          {isSendingOtp ? (
            ...
            <>
              ...
              <PaperPlaneTilt size={15} weight="bold" className="group-hover:translate-x-0.5 transition-transform" />
            </>
          )}
        </button>
      </form>
        ...
        <div>
          <div className="relative flex items-center group">
            <LockKey
              size={18}
              className={`absolute left-1 transition-colors duration-300 pointer-events-none ${
                errors.otp ? "text-red-400" : "text-gray-500 group-focus-within:text-osmo-lime"
              }`}
            />
            <input
              {...register("otp")}
              ...
              className={`w-full bg-transparent border-b pl-9 pr-4 py-2.5 text-sm text-white placeholder-gray-500 focus:outline-none transition-all duration-300 font-sans tracking-[0.25em] rounded-none ${
                errors.otp
                  ? "border-red-400/70 hover:border-red-400"
                  : "border-white/10 hover:border-white/20 focus:border-osmo-lime"
              }`}
        ...
        <div>
          <div className="relative flex items-center group">
            <LockSimple
              size={18}
              className={`absolute left-1 transition-colors duration-300 pointer-events-none ${
                errors.newPassword ? "text-red-400" : "text-gray-500 group-focus-within:text-osmo-lime"
              }`}
            />
            <input
        ...
        <div>
          <div className="relative flex items-center group">
            <LockSimple
              size={18}
              className={`absolute left-1 transition-colors duration-300 pointer-events-none ${
                errors.confirmPassword ? "text-red-400" : "text-gray-500 group-focus-within:text-osmo-lime"
              }`}
            />
            <input
...
export default function AuthModal({ isOpen = true, onClose }) {
  // 'loginregister' | 'forgot' | 'reset'
  const [view, setView] = useState("loginregister");
  ...
  const handleClose = useCallback(() => {
    // Reset view state before closing so the modal opens fresh next time
    setView("loginregister");
    setResetEmail("");
    if (onClose) {
      onClose();
    } else {
  ...
  // Reset is handled inside handleClose — no effect needed.
        ...
        >
          ...
          >
            ...
            <button
              ...
              className="absolute top-5 right-5 w-8 h-8 rounded-full text-gray-400 hover:text-white hover:bg-white/10 flex items-center justify-center transition-colors cursor-pointer z-10"
            >
